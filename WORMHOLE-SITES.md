# Wormhole PvE Combat Site Intelligence (C1 - C5)

This guide documents the "Sleeper" combat anomalies found in wormhole space. Use this to prepare your tank and prioritize triggers.

## 0. General Rules for All Classes
- **Damage Types:** Sleepers deal **Omni damage** (Equal parts EM, Thermal, Kinetic, and Explosive). Your tank must be uniformly balanced and should not have any "resist holes."
- **Resistances:** Sleepers also have **Omni resists**, meaning you can use any damage type against them without a penalty. However, missiles and drones are popular because they can easily switch to whatever you're best trained in.
- **Triggers:** Killing the "Trigger" ship instantly spawns the next wave. **Never kill the trigger first** unless you want to be overwhelmed.
- **Point/Web:** In C2 and higher, frigates will frequently Warp Scramble or Web you.

---

## Class 1 (C1)
*Entry-level sites. Can be soloed in Destroyers or Passive Shield Gila/Drake.*

### Typical Site: Perimeter Ambush Point
- **Difficulty:** Easy (~5-10m ISK blue loot).
- **Waves:** 3
- **Triggers:**
    - Wave 1: Emergent Defender (Frigate)
    - Wave 2: Emergent Defender (Frigate)
- **Ships:** Algos, Sunesis, or any T1 Cruiser.

---

## Class 2 (C2)
*Step up in difficulty. Ideal for T1 Cruisers or Battlecruisers.*

### Typical Site: Perimeter Checkpoint
- **Difficulty:** Moderate (~15-20m ISK blue loot).
- **Waves:** 4
- **Triggers:**
    - Wave 1: Awakened Defender (Cruiser)
    - Wave 2: Awakened Defender (Cruiser)
    - Wave 3: Awakened Defender (Cruiser)
- **Ships:** Gila, Drake, Confessor.

---

## Class 3 (C3)
*The "Sweet Spot" for solo pilots. High profit, manageable risk.*

### Site: Fortification Frontier Stronghold
- **Blue Loot:** ~41m ISK.
- **Special Info:** No Warp Scrambling. Safe to escape if needed.
- **Waves:** 3
- **Triggers:**
    - Wave 1: Awakened Defender (Cruiser)
    - Wave 2: Awakened Upholder (Cruiser)
    - Wave 3 (Final): Awakened Preserver (Cruiser) - **Note:** Killing this early despawns the site.
- **Recommended Ships:** Passive Shield Gila, Battle-fit Praxis (very popular for C3).

### Site: Outpost Frontier Stronghold
- **Blue Loot:** ~45m ISK.
- **Special Info:** No Energy Neutralization (Neuts). Best for cap-dependent ships.
- **Triggers:**
    - Wave 1: Sleepless Defender (Battleship)
    - Wave 2: Awakened Defender (Cruiser)
- **Recommended Ships:** Gila, Praxis, Myrmidion.

---

## Class 4 (C4)
*Difficult solo due to extreme spawn distances (up to 100km). Better in pairs.*

### Typical Site: Frontier Barracks
- **Blue Loot:** ~80-100m ISK.
- **Waves:** 4
- **Triggers:**
    - Wave 1: Awakened Patroller
    - Wave 2: Awakened Patroller
    - Wave 3: Sleepless Sentinel (Battleship)
- **Recommended Ships:** Multi-boxed Gilas, Rattlesnake, Marauder (Vargur/Paladin).

---

## Class 5 (C5)
*End-game PvE. High damage, heavy neuting, massive logistics required.*

### Site: Core Garrison
- **Blue Loot:** ~250m ISK.
- **Special Info:** Features "Capital Escalations." If a Dreadnought/Carrier is on grid, extra waves of Sleepless Guardians spawn.
- **Waves:** 4
- **Triggers:**
    - Wave 1: Awakened Sentinel (Cruiser)
    - Wave 2: Emergent Keeper (Frigate)
    - Wave 3: Sleepless Sentinel (Battleship)
- **Recommended Ships:** 
    - **Solo:** High-end Marauder (Paladin/Vargur/Golem) with heavy capacitor injection.
    - **Group:** Tinker Rattlesnakes (Cap-recharging each other) or Nestors.

---

## Summary Cheat Sheet

| WH Class | Difficulty | Avg Blue Loot | Primary Danger | Ship Recommendation |
| :--- | :--- | :--- | :--- | :--- |
| **C1** | Low | 8m | Low | Destroyer / T1 Cruiser |
| **C2** | Low-Mid | 18m | Scrams | Gila / Confessor |
| **C3** | Mid | 45m | Neuts (site dep) | Praxis / Gila |
| **C4** | High | 90m | Distance/Neuts | Dual Gila / Rattlesnake |
| ****C5** | Very High | 300m+ | Heavy Neuts/Alpha | Marauder / Tinker Setup |

## Gas Harvesting Intelligence (Fullerenes)

Wormhole gas (Fullerenes) is essential for Tech 3 production. Value varies wildly—some sites are worth 5 minutes of your time, others are worth hundreds of millions.

### Gas Value Table (Approx. Market Values)
*Sorted by Value per m3 (Efficiency)*

| Gas Type | Volume | Avg Unit Price | **Value / m3** | Primary Sites | WH Class |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Fullerite-C320** | 5 m3 | 90,000 ISK | **18,000 ISK** | Instrumental Core | C5, C6, Shattered |
| **Fullerite-C540** | 10 m3 | 45,000 ISK | **4,500 ISK** | Vital Core | C5, C6, Shattered |
| **Fullerite-C32** | 5 m3 | 13,000 ISK | **2,600 ISK** | Vast/Bountiful Frontier | C3 - C6 |
| **Fullerite-C28** | 2 m3 | 3,000 ISK | **1,500 ISK** | Vast/Bountiful Frontier | C3 - C6 |
| **Fullerite-C72** | 2 m3 | 1,200 ISK | **600 ISK** | Perimeter Reservoirs | C1 - C4 |
| **Fullerite-C84** | 2 m3 | 1,100 ISK | **550 ISK** | Perimeter Reservoirs | C1 - C4 |
| **Fullerite-C70** | 1 m3 | 400 ISK | **400 ISK** | Perimeter Reservoirs | C1 - C4 |
| **Fullerite-C60/50** | 1 m3 | < 300 ISK | **< 300 ISK** | Perimeter Reservoirs | C1 - C4 |

### Clearing the Guards (Sleeper Rats)

Gas sites are often guarded by Sleepers, but they don't always spawn immediately.

| Site Category | Spawn Time | Difficulty | Guard Composition | Recommended Ship |
| :--- | :--- | :--- | :--- | :--- |
| **Perimeter (C1-C2)** | ~15-20 Min | Low | Frigates / Cruisers | Destroyer or T1 Cruiser |
| **Ordinary Perim.** | **IMMEDIATE** | **TRAP** | 5x High-Damage Sentries | **DO NOT Nninja.** Need T3C/BC. |
| **Frontier (C3-C4)** | ~15-20 Min | Medium | Cruisers / Frigates | Drake / Gila / Praxis |
| **Core (C5-C6)** | 0 - 5 Min | **High** | 2x-4x Sleeper Battleships | Marauder / Rattlesnake |

### Pro-Tips for Gas Huffers
1. **The "Ninja" Window:** Most Perimeter and Frontier gas sites have a **15-20 minute timer** after you warp in before the Sleeper sentries spawn. You can "Ninja" huff the most valuable gas and warp out before they arrive.
2. **Beware the Ordinary:** The "Ordinary Perimeter Reservoir" is a famous trap. Five high-alpha sentry towers spawn immediately. Never warp a Venture here unless it's already cleared.
3. **Instrumental/Vital:** These sites in C5/C6 are the most dangerous. They spawn Sleeper Battleships that will instantly pop a mining frigate. They are worth the most but require a dedicated combat ship to clear.
4. **C320 is King:** If you find an Instrumental Core Reservoir in a C5/C6, it is often worth more than the combat sites themselves.

---
*Reference: EVE University Wiki & Community Data 2026*
